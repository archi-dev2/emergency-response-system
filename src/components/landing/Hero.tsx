...
function Marquee() {
  const { t } = useLanguage();
  const tickerItems = t.hero?.ticker ?? TICKER_ITEMS;
    ...
    <div className="relative overflow-hidden border-t border-white/10 bg-white/[0.03] py-2.5">
      ...
      >
        {items.map((item, i) => (
          <span
            key={i}
            className="text-xs font-medium text-white/50 flex items-center gap-12"
          >
            {item}
...
/* ── Hero stats — values are static; labels come from translations ── */
const HERO_STAT_VALUES = ["50K+", "200+", "<4 min", "99.9%"];
...
export default function Hero() {
  ...
  return (
    <section
      ...
      style={{
        background:
          "linear-gradient(135deg, #050810 0%, #0a0e1a 40%, #0c0814 100%)",
      }}
    >
      <ParticleCanvas />
      <AmbientOrbs />
      ...
      >
        <div className="flex min-h-screen flex-col items-center justify-center pt-20 pb-16">
          ...
          >
            ...
            >
              ...
              <span
                style={{
                  background:
                    "linear-gradient(135deg, #ef4444 0%, #ec4899 40%, #8b5cf6 100%)",
          ...
          >
            ...
            <div className="flex items-center gap-1.5">
              <HeartPulse className="size-3.5 text-white/20" />
              {t.hero.trustActive}
            </div>
            <div className="w-px h-4 bg-white/10" />
            <div className="flex items-center gap-1.5">
              <Activity className="size-3.5 text-white/20" />
              {t.hero.trustRealtime}
            </div>
          </motion.div>
            ...
            <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
              ...
              >
                ...
                >
                  ...
                  {/* Phone frame */}
                  <div
                    className="relative z-10 w-52 h-96 sm:w-60 sm:h-[28rem] rounded-[2.5rem] overflow-hidden"
                    style={{
                      background:
                        "linear-gradient(160deg, rgba(255,255,255,0.08) 0%, rgba(255,255,255,0.03) 100%)",
                      border: "1px solid rgba(255,255,255,0.12)",
                      boxShadow:
                        "0 40px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.06) inset",
                    }}
                  >
                    {/* Notch */}
                    ...
                    <div className="flex flex-col items-center justify-center mt-8 gap-4 px-4">
                      ...
                      <motion.div
                        ...
                        style={{
                          ...
                          boxShadow:
                            "0 0 40px rgba(239,68,68,0.6), 0 0 80px rgba(239,68,68,0.2)",
                        }}
                        animate={{
                          boxShadow: [
                      ...
                      >
                        <p className="text-[9px] text-white/30 uppercase tracking-wider mb-1.5">
                          {t.hero.phoneProfileLabel}
                        </p>
                        <div className="flex gap-2 flex-wrap">
                          {["O+", "Penicillin ⚠", "Metformin"].map((tag) => (
                            <span
                              key={tag}
                              className="text-[8px] px-1.5 py-0.5 rounded bg-white/10 text-white/60"
                            >
                              {tag}
                            </span>
                          ))}
              ...
              <div className="flex-1 grid grid-cols-2 gap-4 w-full lg:w-auto">
                {[
                  { value: "50K+", label: t.hero.statLivesSaved },
                  { value: "200+", label: t.hero.statHospitals },
                  { value: "<4 min", label: t.hero.statResponse },
                  { value: "99.9%", label: t.hero.statUptime },
                ].map((s, i) => (
                  <motion.div
                    key={s.label}
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="text-xs text-white/40 font-medium mt-1 uppercase tracking-wider">
                        {s.label}
                      </div>
                    </div>
                  </motion.div>
